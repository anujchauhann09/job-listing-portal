const multer = require('multer');
const path = require('path');

const { FILE_CONSTANTS } = require('./file.constants');
const AppException = require('@/exceptions/app.exception');
const { HTTP_STATUS } = require('@/constants/http-status');
const { LOGO_UPLOAD_DIR } = require('./file.logo.config');

const storage = multer.diskStorage({
  destination: (_req, _file, cb) => cb(null, LOGO_UPLOAD_DIR),
  filename: (req, file, cb) => {
    const ext = path.extname(file.originalname);
    const filename = `logo-${req.user.sub}-${Date.now()}${ext}`;
    cb(null, filename);
  }
});

const fileFilter = (_req, file, cb) => {
  if (!FILE_CONSTANTS.LOGO.ALLOWED_MIME_TYPES.includes(file.mimetype)) {
    return cb(
      new AppException({
        status: HTTP_STATUS.BAD_REQUEST,
        message: 'Only PNG, JPG, JPEG, WEBP images are allowed'
      }),
      false
    );
  }
  cb(null, true);
};

const uploadEmployerLogo = multer({
  storage,
  fileFilter,
  limits: {
    fileSize: FILE_CONSTANTS.LOGO.MAX_SIZE_MB * 1024 * 1024
  }
});

module.exports = { uploadEmployerLogo };
